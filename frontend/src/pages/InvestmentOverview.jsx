...
const events = [
  ...
  const activeProjects = projects.filter((p) => !isInactiveProject(p));
  ...
  const totals = projects.reduce(
    (acc, p) => {
      ...
      const daily =
        !inactive && Number.isFinite(tableDaily) && tableDaily > 0
          ? tableDaily
          : !inactive
            ? Number(p.per_day) || 0
            : 0;
    ...
    return (
      <div key={project.id}>
        ...
        {isExpanded && project.categories && project.categories.length > 0 && (
          <div
            className="ml-10 mt-2 mb-2 space-y-1.5"
            data-testid={`project-categories-${project.id}`}
          >
            {[...project.categories]
  ...
  const renderInactiveProjectCard = (project) => {
    ...
    return (
      <Card
        key={project.id}
        className="border-border/30 bg-card/60 opacity-70"
            ...
            <div className="flex items-center gap-3">
              {project.icon_url ? (
                <img
                  src={project.icon_url}
                  alt=""
                  className="w-7 h-7 rounded-md object-contain grayscale"
                />
              ) : (
                <div className="w-7 h-7 rounded-md bg-secondary flex items-center justify-center">
                  <span className="text-xs font-bold text-muted-foreground">
          ...
          <CardContent className="p-5">
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
              ...
              <div>
                ...
                <p className="font-mono text-sm font-medium text-foreground">
                  {formatCurrency(totals.per_week)}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="font-mono text-sm font-medium text-foreground">
                  {formatCurrency(totals.per_year)}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          {[...activeProjects]
            .sort(
              (a, b) =>
                (Number(b.earned) || 0) -
                (Number(b.invested) || 0) -
                ((Number(a.earned) || 0) - (Number(a.invested) || 0))
            )
