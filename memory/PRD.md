...
- **Custom sub-category labels on integrations (Feb 2026 — latest)** — Every integration sync now supports a custom "label" field that auto-populates as a sub-category in the Investment Overview, so users can tag earnings as "Mining", "Bonus", etc. instead of the default. Coverage:
  - Acurast & RollerCoin: per-update label input (visible when delta>0 and action=earning).
  - Unity Network: per-update label input (data-testid=`unity-network-label-input`), passed through `applyUnityNetworkBalanceUpdate({label})`.
  - Nosana: label persisted in config (since it auto-syncs daily) via `nosana-label-input` data-testid; `nosanaSync.js` reads `config.label` for both `addTransaction` category and `addToCategory`.
  - GoMining: per-update single label override (data-testid=`gomining-label-input`) that overrides the default `GoMining (GMT)` / `GoMining (Boost)` / `GoMining (BTC)` for whichever sides moved this update.
  - Custom Integration card already supports labels by design.
