...
from ..schemas import spending_transaction_to_dict
...
def get_cipher() -> Fernet:
    ...
    if _cipher is None:
        ...
        if not key:
            raise HTTPException(
                status_code=503,
                detail="APP_ENCRYPTION_KEY is not set on the backend.",
            )
...
def normalize_access_url(access_url: str) -> tuple[str, str | None]:
    ...
    Also forces SimpleFIN to include pending transactions.
    """
    ...
    # SimpleFIN only returns pending transactions when requested.
...
def normalize_transaction_date(
    ...
    if (
        ...
        # Pending SimpleFIN transactions can
        # sometimes use posted=0.
        if timestamp <= 0:
            return None
...
def save_simplefin_payload(
    ...
    for account in accounts:
        ...
        for transaction in transactions:
            ...
            pending = (
                simplefin_transaction_is_pending(
                    transaction
                )
            )

            tx_key = (
                simplefin_transaction_key(
                    account_key,
                    merchant,
                    str(posted)[:10],
                    amount,
                ...
                existing_transaction = (
                    db.scalar(
                        select(
                            ...
                            SpendingTransaction.user_id
                            == user.id,
                            SpendingTransaction.account_id
                            == account_key,
                            SpendingTransaction.source
                            == "simplefin",
...
async def browser_sync(
    ...
    connection = db.scalar(
        select(
            ...
            SimplefinConnection.id
            == request.connection_id,
            SimplefinConnection.user_id
            == user.id,
        )
    )
...
async def claim_setup_token(
    ...
    for the HTTPS access URL.
    """
    setup_token = (
        request.setup_token.strip()
    )
...
async def sync_simplefin(
    ...
    for connection in connections:
        payload = fetch_simplefin(
            read_access_url(
                connection
            )
        )

        for account in payload.get(
            "accounts",
            [],
        ):
            account_count += 1
            ...
            for transaction in account.get(
                "transactions",
                [],
            ):
                transaction_id = (
